...
function simulatePeriod(g) {
  ...
  if (seniorClub && doFinish && apps >= 5) {
    ...
    if (finish === 1) {
      ...
      if (club.div > 1) { club.div -= 1; club.req += 6; relabelLeague(g, club); }
      shiftReaction(g, 1, '리그 우승');
    } else if (club.div > 1 && finish <= 2) {
      ach.push('승격'); club.div -= 1; club.req += 6; relabelLeague(g, club); g.flags.justPromoted = club.name;
    } else if (finish >= size - 1) {
      ach.push('강등'); if (club.div < 3) { club.div += 1; club.req -= 6; relabelLeague(g, club); }
  ...
  choices: (g) => {
    ...
    if (!rich) list.push({
      ...
      fx: (gg) => {
        ...
        gg.npcs.father.status = '실직'; gg.npcs.father.statusYear = gg.world.year;
...
ev({
  ...
  choices: (g) => {
    ...
    if (g.player.club) {
      ...
      // 이적 요청 — 구단이 그 자리에서 답한다
      list.push({ t: '"이적 요청하자. 여기서는 더 안 늘어."',
...
ev({
  ...
  body: (g) => {
    ...
    const offers = accepted ? generateOffers(g, 5) : [];
  ...
  body: (g) => {
    ...
    return g.flags._wcIn
      ? `${g.world.year} 월드컵 ${NT_NAME[g.player.ntTeam]} 최종 명단이 발표됐다. 이름이 있다.\n\n` +
        `${club.name}(${club.league}) 소속으로 뽑혔다. 소집 명단에는 나보다 유명한 이름이 스무 개 넘게 있다.`
...
export function classify(g) {
  ...
  if (div === 1 && apps > 330) return 'T3';                                  // 1부 정착
...
export function careerTable(g) {
  ...
  for (const s of g.player.career.seasons) {
    ...
    if (prev) {
      ...
      rows.set(key, {
        ...
        finish: s.finish, size: s.size, ucl: s.ucl, ntCaps: s.ntCaps, ntName: s.ntName,
      });
    }
  }
  return [...rows.values()].map((r) => {
    ...
    return {
      ...
      uclText: r.ucl || '-',
      ntText: r.ntCaps ? `${r.ntName} ${r.ntCaps}캡` : '-',
    };
  });
}
